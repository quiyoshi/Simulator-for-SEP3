...
public class CPU {
	...
	public CPU(Bus addrBus, Bus dataBus) {
		...
		regs[REG_ISR] = new ISRegister(decoder);
		regs[REG_SC]  = new Register();
		alu           = new ALU(aBus, bBus, sBus, regs[REG_PSW], iset);

		// A バスにレジスタを接続
		for (int i = 0; i <= REG_R7; ++i) { getABusSelector().addFrom(i); }
		getABusSelector().addFrom(REG_MDR);
		// B バスにレジスタを接続
		getBBusSelector().addFrom(REG_B0);
		// S バスにレジスタを接続
		for (int i = 0; i <= REG_R7; ++i) { getSBusSelector().addTo(i); }
	...
	public void clock() {
		for (int i = 0; i <= REG_ISR; ++i) { regs[i].clock(); }
	}
	// CPUリセット
	public void reset() {
		for (int i = 0; i < REG_R6; ++i) { regs[i].setValue(0); }
		//regs[REG_R0].setValue(0xC013);
		//regs[REG_R1].setValue(0x0001);
		regs[REG_R6].setValue(0x0010);
		regs[REG_R7].setValue(0xF800);
		regs[REG_B0].setValue(0);
		regs[REG_MAR].setValue(0);
		regs[REG_MDR].setValue(0);
		regs[REG_ISR].setValue(0);
		aBus.setValue(0);
		bBus.setValue(0);
		sBus.setValue(0);
		halted.off();
	...
	public void powerOff() {
		for (int i = 0; i <= REG_ISR; ++i) { regs[i].setValue(0); }
		regs[REG_SC].setValue(0);
		aBus.setValue(0);
		bBus.setValue(0);
		sBus.setValue(0);
		halted.off();
	...
	public void setRegister(int i, int value) {
		regs[i].setValue(value);
	}
	// 命令実行サイクルのどの状態にあるかを得る、セットする
	public State getCurrentState()		{ return currentState; }
	public void setCurrentState(State s) {
		currentState = s;
		if (s == sf.getState(StateFactory.SC_HLT)) { halted.on(); }		// HLT状態ならビューに伝えるための変数をセット
		if (s == sf.getState(StateFactory.SC_ILL)) { illegal.on(); }	// ILL状態なら（以下同じ）
	}
}
